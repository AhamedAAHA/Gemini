...
export async function analyzeBill(input: AnalyzeInput): Promise<AnalysisResult> {
  let text = input.text ?? "";
  ...
  const semanticFlags = await runSemanticPass(bill);
  flags.push(...semanticFlags);
  flags.sort((a, b) => b.overcharge - a.overcharge);
  ...
  const result: AnalysisResult = {
    ...
    parsedBy,
    analyzedBy: semanticFlags.length > 0 ? "rules+semantic" : "rules",
  ...
  try {
    ...
    // analysis still returns even if persistence is unavailable
  }
